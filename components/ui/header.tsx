...
import { ChevronDown, Menu, X, Atom, CircleDivide, NotebookPen, Landmark } from 'lucide-react';
import { ModeToggle } from '../ModeToggle';
...
export default function Header() {
    ...
    return (
        <header className="w-[90%] px-4 py-6 max-w-[1920px] mx-auto">
            <div className="flex items-center justify-between xl:grid xl:grid-cols-3">
                ...
                {/* Desktop Navigation */}
                <ul className="hidden xl:flex justify-center items-center gap-10 font-secondary dark:text-[#888888] font-base">
                    <li>
                        <DropdownMenu>
                            <DropdownMenuTrigger className="flex items-center gap-1 cursor-pointer">Products <ChevronDown /></DropdownMenuTrigger>
                            <DropdownMenuContent>
                                <DropdownMenuLabel></DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem><Atom />Science</DropdownMenuItem>
            ...
            {/* Mobile Menu */}
            {menuOpen && (
                <div className="mt-4 lg:hidden space-y-6">
                    <ul className="flex flex-col gap-4 text-base font-secondary dark:text-[#888888]">
                        <li>
                            <DropdownMenu>
                                <DropdownMenuTrigger className="flex items-center gap-1 cursor-pointer">Products <ChevronDown /></DropdownMenuTrigger>
                                <DropdownMenuContent>
                    ...
                    <div className="flex flex-col gap-3">
                        <Button className="bg-white border border-display text-display hover:text-gray-50 py-4 px-4 text-lg" asChild>
                            <LoginLink postLoginRedirectURL="/about">Sign in</LoginLink>
                        </Button>
                        <Button className="bg-display dark:bg-[#2ca58d] px-4 py-4 text-gray-50 text-lg">Try now!</Button>
                        <ModeToggle />
                    </div>
                </div>
            )}
